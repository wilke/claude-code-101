...
- **Exercises** — three parallel tracks (`exercises-opt/`, `exercises-pde/`, `exercises-lin_alg/`), each with the same numbered progression `01-claude-md` … `04-logbook`; only the optimization track adds `06-capstone`. Each folder is opened on its own (`cd … && claude`) with steps in its `README.md`. A track's `SOLUTION.md` is deliberately kept **out of** the directory where `claude` runs so the model can't read ahead — preserve that when editing exercises. Exercise Python deps: `numpy scipy matplotlib pytest`.
