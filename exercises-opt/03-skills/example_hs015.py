...
# fo-checker skill: validate every Uno solve against an independent KKT check.
sys.path.insert(0, os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                ".claude", "skills", "fo-checker"))
from check_fo import fo_residuals, report, dense_jacobian
...
if __name__ == '__main__':
	...
	# Describe the NLP for the fo-checker skill, reusing the very callables and
	# bound data handed to Uno (dense Jacobian rebuilt from the sparse COO pattern).
	def _grad(x):
		g = np.zeros(number_variables); objective_gradient(np.asarray(x, float), g); return g
	def _cons(x):
		cv = np.zeros(number_constraints); constraints(np.asarray(x, float), cv); return cv
	nlp = {
		"n": number_variables, "m": number_constraints, "grad": _grad, "c": _cons,
		"jac": dense_jacobian(jacobian, jacobian_row_indices, jacobian_column_indices,
		                      number_jacobian_nonzeros, number_constraints, number_variables),
		"lx": np.array(variables_lower_bounds), "ux": np.array(variables_upper_bounds),
		"lc": np.array(constraints_lower_bounds), "uc": np.array(constraints_upper_bounds),
	}
	# 1e-4 matches this example's objective tolerance: interior-point presets (ipopt)
	# converge along the central path and leave a small complementarity residual ~ the
	# barrier parameter, so a strict 1e-6 KKT tolerance would (correctly) flag them.
	def validate(result, label, tol=1e-4):
		print(f"\n--- fo-checker: validating Uno result ({label}) ---")
		# Once Uno introduces constraint slacks (e.g. the exact-Hessian runs) it returns
		# bound duals of length n+m: the first n are the variable-bound multipliers we want,
		# the trailing m belong to the slacks. Slice to the original-space duals.
		n, mm = number_variables, number_constraints
		report(fo_residuals(nlp, result.primal_solution[:n], result.constraint_dual_solution[:mm],
		                    result.lower_bound_dual_solution[:n], result.upper_bound_dual_solution[:n],
		                    tol=tol), tol)
	...
	validate(result, "run 1: filtersqp / L-BFGS")
	...
	validate(result, "run 2: filtersqp / exact Hessian")
	...
	print("Method used:", uno_solver.get_method_description())
	assert abs(result.solution_objective - 306.5) <= 1e-4
	validate(result, "run 3: ipopt / MUMPS")
	...
	print("Method used:", uno_solver.get_method_description())
	assert abs(result.solution_objective - 306.5) <= 1e-4
	validate(result, "run 4: filterslp / HiGHS")
